...
constexpr int OBJECT_INSTANCES = SIDE*SIDE*SIDE;
//constexpr int OBJECT_INSTANCES = pow(10, 3);
...
public:
    ...
    // TEST: Store random per-object rotations
	...
	void prepareUniformBuffers()
	{
		...
		std::normal_distribution<float> rndDist(0.0f, 1.0f); // mean, stddev
		for (uint32_t i = 0; i < OBJECT_INSTANCES; i++) {
            ...
            // TEST:
            positions[i] = glm::vec3(rndDist(rndEngine), rndDist(rndEngine), rndDist(rndEngine));
            //positions[i] = glm::vec3(1.0f, 1.0f, 1.0f);
		}
		...
		for (uint32_t x = 0; x < dim; x++)
		{
			for (uint32_t y = 0; y < dim; y++)
			{
				for (uint32_t z = 0; z < dim; z++)
				{
                    ...
                    // TEST:
                    //float scatter = 5.0f;
                    pos.x += positions[index].x * scatter;
                    pos.y += positions[index].y * scatter;
                    pos.z += positions[index].z * scatter;
	...
	virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay)
	{
		if (overlay->header("Settings")) {
			if (overlay->checkBox("Spin", &spin)) {
				//buildCommandBuffers();
			}
            if (overlay->checkBox("Identity", &identity)) {
				//buildCommandBuffers();
			}
			if (overlay->inputFloat("Scale", &scale, 0.1f, 2)) {
				//updateUniformBuffersBlur();
			}
            if (overlay->inputFloat("Scatter", &scatter, 0.1f, 2)) {
				//updateUniformBuffersBlur();
			}
		}
	}
...
// RG: macro expansion werkt maar we krijgen een unrecognized token voor: @autoreleasepool
// Kennelijk wordt de file niet als Objective-C file gecompiled terwijl de compiler flags
// er wel staan.
VULKAN_EXAMPLE_MAIN()
